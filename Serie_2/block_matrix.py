...
def plot_relative_zeroes(n_array):
    """
    Plots the amount of non zeros values contained in the block matrix for each
    dimension and a given array of n's.
    ...
    for d in [1, 2, 3]:
        ...
        for n in n_array:
            ...
            print("d:", d, "; n:", n, "; relative non zeros:",
                  matrix.eval_zeros()[2], "; absolute non zeros:", matrix.eval_zeros()[0])
        plt.plot(n_array, rel_non_zeros, "b.", label='relative number of non zero values')
        plt.xlabel('$n$')
        plt.title('d = ' + str(d))
        plt.legend()
        plt.grid()
