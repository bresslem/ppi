...
Rechnet der Computer also mit der gegebenen Formel $(D_h^{(1)}f)(x) = \frac{f(x+h)-f(x)}{h}$ die erste finite Differenz aus und liegt $f(x+h)$ genügend nah an $f(x)$, dann reicht die Genauigkeit der Maschinenzahlen mit 52 Bit Mantisse nicht mehr aus, um den Unterschied zwischen beiden beliebig genau zu berechnen und die Genauigkeit der Rechnung sinkt wieder.
...
Analog werden sie bei größer werdenden $j$'s immer größer.
Bei kleiner werdenden $j$'s befindet sich das "`optimale"' $h$, bei dem die Fehler am geringsten sind schon weiter rechts, es ist also größer, da wir hier mit immer kleineren Zahlen hantieren, die Auslöschung tritt also schon früher ein.
Genauso verschiebt sie sich bei großen Werten für $j$ nach links, da dort die Auslöschung erst später zu tragen kommt.
