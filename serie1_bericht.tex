...
\begin{shaded} Eine Funktion $f:D \rightarrow \mathbb{R}$ (mit $D\subset \mathbb{R}$) heißt differenzierbar an der Stelle $x_0 \in D$, falls folgender Grenzwert existiert (mit $(x_0+h \in D$): \[f'(x_0) = \lim _{x\to x_0} {\frac {f(x)-f(x_0)}{x-x_{0}}} = \lim _{h\to 0} {\frac {f(x_0+h)-f(x_0)}{h}}\] Dieser Grenzwert heißt \textit{Differentialquotient} / \textit{Ableitung} von $f$ nach $x$ an der Stelle $x_0$. \end{shaded} \\
...
(GRAFIK?)
Setzt man $\Delta x =: h$ und lässt $h$ gegen $0$ laufen, erhält man den Differentialquotienten, sprich die Ableitung von $f$ an der Stelle $x_0$. Allerdings ist der Grenzübergang der sogenannten \textit{Schrittweite} $h$ gegen $0$ für den Computer nicht machbar. 
numerisch nicht möglich, da es im Rechner zu einem Overflow kömmen kann.\\
